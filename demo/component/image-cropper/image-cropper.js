Component({
  ...
  attached() {
    ...
    this.data.containerWidth =
      this.properties.container_width ?? this.data.containerWidth
    this.data.containerHeight =
      this.properties.container_height ?? this.data.containerHeight

    this.setData({
      _canvas_height: this.data.height,
      _canvas_width: this.data.width,
      _img_top: this.data.containerHeight / 2,
      _img_left: this.data.containerWidth / 2,
    })
    this._initCanvas()
  ...
  methods: {
     ...
     */
    setCutCenter() {
      let cut_top = (this.data.containerHeight - this.data.height) * 0.5
      let cut_left = (this.data.containerWidth - this.data.width) * 0.5
      //顺序不能变
      this.setData({
    ...
    _initImageSize() {
      ...
      if (
        ...
        this.data.INIT_IMGWIDTH = this.data.img_width =
          (this.data.containerWidth / 100) * width
      }
      if (
        ...
        this.data.INIT_IMGHEIGHT = this.data.img_height =
          (this.data.containerHeight / 100) * height
      }
    },
    /**
    ...
    _cutDetectionPosition() {
      let _cutDetectionPositionTop = () => {
          ...
          if (
            this.data.cut_top >
            this.data.containerHeight - this.data.height
          ) {
            this.setData({
              cut_top: this.data.containerHeight - this.data.height,
            })
          }
        },
        _cutDetectionPositionLeft = () => {
          ...
          if (this.data.cut_left > this.data.containerWidth - this.data.width) {
            this.setData({
              cut_left: this.data.containerWidth - this.data.width,
            })
          }
        }
      ...
      if (this.data.cut_top == null && this.data.cut_left == null) {
        ...
        _cutDetectionPositionTop()
        this.setData({
          cut_left: (this.data.containerWidth - this.data.width) / 2,
        })
      } else if (this.data.cut_top == null && this.data.cut_left != null) {
        _cutDetectionPositionLeft()
        this.setData({
          cut_top: (this.data.containerHeight - this.data.height) / 2,
        })
      }
    },
    /**
    ...
    _canvasDetectionPosition() {
      if (this.data.canvas_top == null && this.data.canvas_left == null) {
        ...
        if (
          ...
          this.data.canvas_top > this.data.containerHeight
        ) {
        ...
        if (
          ...
          this.data.canvas_left > this.data.containerWidth
        ) {
          this.data._canvas_overflow = true
        } else {
          this.data._canvas_overflow = false
    ...
    //改变截取框大小
    _computeCutSize() {
      if (this.data.width > this.data.containerWidth) {
        this.setData({
        ...
        this.setData({
          cut_left: this.data.containerWidth - this.data.cut_left,
        })
      }
      if (this.data.height > this.data.containerHeight) {
        this.setData({
        ...
        this.setData({
          cut_top: this.data.containerHeight - this.data.cut_top,
        })
      }
      !this.data._canvas_overflow && this._draw()
    },
    ...
    _click(event) {
      ...
      this._draw(() => {
        ...
        /** 修正点击坐标 */
        x -= this.properties.container_offset_left
        y -= this.properties.container_offset_top
      ...
      if (this.data._flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
        ...
        height =
          ...
          (this.data.CUT_START.corner > 1 && this.data.CUT_START.corner < 4
            ? 1
            : -1) *
            (this.data.CUT_START.y -
              (e.touches[0].clientY - this.properties.container_offset_top))
        switch (this.data.CUT_START.corner) {
          case 1:
            ...
            width =
              this.data.CUT_START.width +
              this.data.CUT_START.x -
              (e.touches[0].clientX - this.properties.container_offset_left)
    ...
    _cutTouchStart(e) {
      ...
      let currentX =
        e.touches[0].clientX - this.properties.container_offset_left
      let currentY = e.touches[0].clientY - this.properties.container_offset_top
